...
function auto_tech:set_tech_unit()
    ...
    self.technology_nodes:for_all_nodes(function(technology_node)
        ...
        if science_pack_unlocked_by_this_tech then
            ...
            if verbose_logging then
                log("Depth of a new science pack tech is " .. depths_of_science_packs[#depths_of_science_packs])
            end
            ::continue::
        end
    end)
    ...
    self.technology_nodes:for_all_nodes(function(technology_node) 
        ...
        for i,pack_depth in pairs(depths_of_science_packs) do
            if pack_depth >= technology_node.depth then
                if i>2 then
                    relative_depth_in_science_tier = relative_depth_in_science_tier - depths_of_science_packs[i-1]
                end
                break
            end
        end

        local depth_percent = ((technology_node.depth - (depths_of_science_packs[1]+1))/ (max_depth- (depths_of_science_packs[1]+1)))
        ...
        if relative_depth_in_science_tier < 6 and relative_depth_in_science_tier > 0 then
            x= (relative_depth_in_science_tier-1)/4.0
            factorio_tech.unit.count = factorio_tech.unit.count * (0.5*(1.0-x)+x)
        end
        factorio_tech.unit.count = math.max(cost_rounding(factorio_tech.unit.count), 1)
        total_sciece_cost = total_sciece_cost + factorio_tech.unit.count
    ...
    if verbose_logging then
            log("Total cost of non-infinite technologies is: " .. total_sciece_cost )
        end
end
